...
def main():
    ...
    for j in range(args.continue_from_epoch, args.continue_from_epoch+num_updates):
        ...
        if j % save_image_every == 0:
            ...
            summary_writer.add_images('samples_step_{}'.format(j), rollouts.obs[0][0:4])
            plt.show()
        ...
        for step in range(args.num_steps):
            ...
            rollouts.insert(torch.from_numpy(obs), recurrent_hidden_states, action,
                            action_log_prob, value, torch.from_numpy(reward).unsqueeze(1), masks, bad_masks, attn_masks, seeds)
        ...
        if j % args.log_interval == 0 and len(episode_rewards) > 1:
            ...
            print(
                ...
                .format(j, total_num_steps,
                        ...
                        np.median(episode_rewards), np.min(episode_rewards), np.max(episode_rewards), dist_entropy, value_loss,
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            eval_dic_len = {}
            for eval_disp_name in EVAL_ENVS:
                eval_dic_rew[eval_disp_name], eval_dic_len[eval_disp_name] = evaluate_procgen(actor_critic, eval_envs_dic, eval_disp_name,
                                                  args.num_processes, device, steps=args.num_steps)
                ...
                # summary_writer.add_scalar(f'eval_mean/{eval_disp_name}', np.mean(eval_dic[eval_disp_name]),
                #                           (j+1) * args.num_processes * args.num_steps)
                # summary_writer.add_scalar(f'eval_min/{eval_disp_name}', np.min(eval_dic[eval_disp_name]),
                #                           (j+1) * args.num_processes * args.num_steps)
                # summary_writer.add_scalar(f'eval_max/{eval_disp_name}', np.max(eval_dic[eval_disp_name]),
                #                           (j+1) * args.num_processes * args.num_steps)
