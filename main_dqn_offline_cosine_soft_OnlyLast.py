...
def main_dqn(params):
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        #### Update Theta #####
        loss, grad_L2, _, _ = compute_td_loss(
        ...
        #### Update Phi #####
        grad_loss, grad_grads_L2, _, _ = compute_td_loss(
        ...
        # if ((sum(Corr_all_grad_vec) / len(Corr_all_grad_vec)) < min_corr) and (mean_grad_grad.norm(2) > 0.0001) and (mean_grad_val.norm(2) > 0.0001):
            ...
            # while (Corr_L2_mean_attn_grad < 0.9) and iter < 10000 and attn_grads[0].norm(2) > 0.1:
            while (Corr_L2_mean_attn_grad < 0.9) and iter < 10000:
                ...
                grad_loss, grad_grads_L2, _, start_ind = compute_td_loss(
                    agent, params.num_mini_batch, 10*params.mini_batch_size_val, grad_replay_buffer, optimizer,
                    params.gamma, params.loss_var_coeff, train=False
                )
                # grad_losses.append(grad_loss.data)

                attn_grads_grad_loss = torch.autograd.grad(grad_loss,
                                                updated_val_params.values(),
                                                create_graph=False)

                val_loss, val_grads_L2, _, start_ind_val = compute_td_loss(
                    agent, params.num_mini_batch, 10*params.mini_batch_size_val, val_replay_buffer, optimizer,
                    params.gamma, params.loss_var_coeff, train=False
                )
                # val_losses.append(val_loss.data)

                attn_grads_val_loss = torch.autograd.grad(val_loss,
                                                updated_val_params.values(),
                                                create_graph=False)
                ...
                val_loss, val_grads_L2, _, _ = compute_td_loss(
                    agent, params.num_mini_batch, 10*params.mini_batch_size_val, val_replay_buffer, optimizer,
                    params.gamma, params.loss_var_coeff, train=False, same_ind=True, start_ind_array=start_ind_val
                )
                # val_losses.append(val_loss.data)
                ...
                # Corr_all_grad = (grad_mean_grad * val_mean_grad).sum() / (grad_mean_grad.norm(2) * val_mean_grad.norm(2))
                Corr_dqn_L2_grad = (grad_grads_L2 * val_grads_L2).sum() / (grad_grads_L2.norm(2) * val_grads_L2.norm(2))

                # grad_grad_sumQ_attn.append(grad_mean_grad)
                # grad_sumQ_val_attn.append(val_mean_grad)

                grad_grad_L2_sumQ_attn.append(grad_grads_L2)
                ...
                agent.q_network.input_attention._grad = attn_grads_grad_loss[0] + attn_grads_val_loss[0] - loss_corr_coeff*attn_grads_Corr_dqn_L2_grad[0]
                ...
                print("update attention, attn L2 corr {}, mean attn L2 corr {}, grad loss {}, gard norm {}, gard val norm {}, attn_grads norm {}".format(Corr_dqn_L2_grad, Corr_L2_mean_attn_grad, grad_loss, grad_grads_L2.norm(2),val_grads_L2.norm(2),agent.q_network.input_attention._grad.norm(2)))
                ...
                print("grads_grad_loss {}, val_grad_loss {}, minus_grads_Corr_dqn_L2_grad {}".format(attn_grads_grad_loss[0],attn_grads_val_loss[0] , -loss_corr_coeff*attn_grads_Corr_dqn_L2_grad[0]))
                print("attention grad {}".format(agent.q_network.input_attention._grad.data))
