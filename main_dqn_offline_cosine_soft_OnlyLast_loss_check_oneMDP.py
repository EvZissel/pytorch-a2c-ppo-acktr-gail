...
def compute_td_loss(agent, num_mini_batch, mini_batch_size, replay_buffer, optimizer, gamma, loss_var_coeff, train=True, same_ind=False, start_ind_array=None):
    ...
    #                                 updated_train_params.values(),
    #                                 create_graph=create_graph)
    return total_loss, total_grad_L2, start_ind_array
...
def main_dqn(params):
    ...
    logdir = 'offline_soft_OL_loss_check_oneMDP' +  params.env + '_' + str(params.seed) + '_num_arms_' + str(params.num_processes) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
    if params.rotate:
    ...
    a_vec = np.linspace(-9.0, 9.0, num=20)
    ...
    for j in range(10):
        ...
        for idx, i in enumerate(a_vec):
            ...
            Corr_dqn_L2_grad = 0
            for i in range(params.num_processes):
                Corr_dqn_L2_grad_i.append(
                    (grads_L2[i] * val_grads_L2[i]).sum() / (grads_L2[i].norm(2) * val_grads_L2[i].norm(2)))
                Corr_dqn_L2_grad += Corr_dqn_L2_grad_i[i] / params.num_processes
