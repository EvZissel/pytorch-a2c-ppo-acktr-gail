...
def compute_td_loss(agent, num_mini_batch, mini_batch_size, replay_buffer, optimizer, gamma, loss_var_coeff, train=True, same_ind=False, start_ind_array=None):
    ...
    #                                 updated_train_params.values(),
    #                                 create_graph=create_graph)
    return total_loss, grad_L2_states_all, states_all, start_ind_array
...
def main_dqn(params):
    ...
    # scheduler_val = LambdaLR(optimizer_val, step_size=50, gamma=0.1, last_epoch=-1, verbose=True)
    ...
    val_lr_update = 0
    lambda_val = lambda epoch: 0.999 ** val_lr_update
    scheduler_val = LambdaLR(optimizer_val, lr_lambda=[lambda_val], verbose=True)
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        if (Corr_dqn_L2_grad < min_corr) or (ts==0):
        # if ((sum(Corr_all_grad_vec) / len(Corr_all_grad_vec)) < min_corr) and (mean_grad_grad.norm(2) > 0.0001) and (mean_grad_val.norm(2) > 0.0001):
            torch.save(
                {'state_dict': q_network.state_dict(),'target_state_dict': target_q_network.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'optimizer_val_state_dict': optimizer_val.state_dict(),
            ...
            # print("correlation: {}".format(Corr_dqn_L2_grad))
            iter = 0
            # optimizer_val.defaults['lr'] = params.learning_rate_val
            while (Corr_dqn_L2_grad < 0.99) and iter < 1000:
                iter +=1
                ...
                # print("attn_grads_loss {}".format( attn_grads_loss[0]))
