...
def main():
    ...
    logdir = 'Wattn_grad_' + args.env_name + '_' + args.algo + '_seed_' + str(args.seed) + '_num_arms_' + str(args.num_processes) + '_entro_' + str(args.entropy_coef) \
    ...
    # Validation envs
    val_envs = make_vec_envs(args.val_env_name, args.seed, args.num_processes, eval_locations_dic['valid_eval'],
    ...
    val_rollouts = RolloutStorage(args.num_steps, args.num_processes,
                                  ...
                                  actor_critic.recurrent_hidden_state_size, mid=args.hard_attn_mid)
    ...
    v = 0
    v_val = 0
    betas = agent.optimizer.state_dict()['param_groups'][0]['betas']
    betas_val = val_agent.optimizer.state_dict()['param_groups'][0]['betas']
    save_copy = True
    ...
    for j in range(args.continue_from_epoch, args.continue_from_epoch+num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states, attn_masks = actor_critic.act(
                    ...
                    rollouts.masks[step], rollouts.attn_masks[step], deterministic=args.det_val)
            actor_critic.train()
        ...
        mean_grad, grads, shapes, value_loss, action_loss, dist_entropy, dist_l2, F_norms_all, F_norms_gru, F_norms_actor, F_norms_critic, F_norms_cat = agent.update(rollouts)
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states, attn_masks = actor_critic.act(
                    ...
                    val_rollouts.masks[step], val_rollouts.attn_masks[step])
        ...
        val_rollouts.compute_returns(next_value, args.use_gae, args.gamma,
                                args.gae_lambda, args.use_proper_time_limits)
        val_mean_grad, val_grads, val_shapes, val_value_loss, val_action_loss, val_dist_entropy, val_dist_l2, val_F_norms_all, val_F_norms_gru, val_F_norms_actor, val_F_norms_critic, val_F_norms_cat  = \
            val_agent.update(val_rollouts, attention_update=False, make_update=False)
        ...
        m_val = betas_val[0]*m_val +  (1 - betas_val[0]) * val_mean_grad
        v = betas[1]*v + (1 - betas[1]) * (mean_grad ** 2)
        v_val = betas_val[1]*v_val + (1 - betas_val[1]) * (val_mean_grad ** 2)
        ...
        if (j % args.save_grad == 0 or j == args.continue_from_epoch + num_updates - 1):
            if j==0:
                ...
                torch.save({'shapes': val_shapes}, os.path.join(logdir_grad, args.val_env_name + "-epoch-{}-val_shapes.pt".format(j)))
            torch.save({'env {}'.format(i): grad_sum[i] for i in  range(len(grad_sum))}, os.path.join(logdir_grad, args.env_name + "-epoch-{}-grad.pt".format(j)))
            torch.save({'env {}'.format(i): grad_sum_val[i] for i in  range(len(grad_sum_val))}, os.path.join(logdir_grad, args.val_env_name + "-epoch-{}-val_grad.pt".format(j)))
            torch.save({'m': m, 'm_val': m_val, 'v':v, 'v_val':v_val}, os.path.join(logdir_grad, args.val_env_name + "-epoch-{}-optimizer_m_and_v.pt".format(j)))
