...
def main():
    ...
    logdir = os.path.join('runs', logdir_)
    ...
    wandb.init(project="main_PPO", entity="ev_zisselman", config=args, name=logdir_, id=logdir_)
    ...
    for j in range(args.continue_from_epoch, args.continue_from_epoch+num_updates):
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            for eval_disp_name, eval_env_name in EVAL_ENVS.items():
                eval_r[eval_disp_name], eval_actions, num_uniform = evaluate(actor_critic, obs_rms, eval_envs_dic, eval_locations_dic, eval_disp_name, args.seed,
                ...
                if eval_disp_name == 'train_eval' and j % 100 == 0:
                    print(eval_actions.data)
                if j == (args.continue_from_epoch + num_updates - 1):
            ...
            wandb.log({f'eval/train episode reward': np.mean(episode_rewards)}, step=(j + 1) * args.num_processes * args.num_steps)
            wandb.log({f'losses/action': action_loss}, step=(j + 1) * args.num_processes * args.num_steps)
            wandb.log({f'eval/value': value_loss}, step=(j + 1) * args.num_processes * args.num_steps)
            wandb.log({f'eval/entropy': dist_entropy}, step=(j + 1) * args.num_processes * args.num_steps)
            wandb.log({f'eval/l2': dist_l2}, step=(j + 1) * args.num_processes * args.num_steps)
            ...
            if j % args.eval_nondet_interval == 0:
                eval_r_nondet, _, _ = evaluate(actor_critic, obs_rms, eval_envs_dic, eval_locations_dic, 'test_eval', args.seed,
                ...
                summary_writer.add_scalar(f'eval/test non-deterministic', np.mean(eval_r_nondet),
                                          (j + 1) * args.num_processes * args.num_steps)
                wandb.log({f'eval/test non-deterministic': np.mean(eval_r_nondet)},
