...
from models_dqn import DQN, CnnDQN, DQN_softAttn, DQN_softAttn_L2grad, DQN_RNNLast, DQN_RNNLast_analytic, DQN_attention_ARNN
...
def main_dqn(params):
    ...
    if USE_CUDA:
        device = "cuda"
    if not params.debug:
        logdir_ = 'offline_soft_OL_out1_ARNN' +  params.env + '_' + str(params.seed) + '_num_arms_' + str(params.num_processes) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
        if params.rotate:
    ...
    q_network = DQN_attention_ARNN(envs.observation_space.shape, envs.action_space.n, params.zero_ind, recurrent=True, hidden_size=params.hidden_size)
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        if (Corr_dqn_out1_mean < min_corr):
            ...
            out1_grad = (out1.mean(0))*(1-torch.sigmoid(agent.q_network.input_attention))
            val_out1_grad = (val_out1.mean(0))*(1-torch.sigmoid(agent.q_network.input_attention))
            ...
            atten_grad_L2_mean = (out1_vec_mean-out1_val_vec_mean)*(grad_out1_vec_mean-grad_out1_val_vec_mean)
