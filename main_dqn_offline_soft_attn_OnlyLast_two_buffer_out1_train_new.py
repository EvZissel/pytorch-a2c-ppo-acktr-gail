...
def main_dqn(params):
    ...
    if USE_CUDA:
        device = "cuda"
    if not params.debug:
        logdir_ = 'offline_soft_OL_out1_diffB_train' +  params.env + '_' + str(params.seed) + '_num_arms_' + str(params.num_processes) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
        if params.rotate:
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        iter = 0
        while (Corr_dqn_L2_grad_mean < min_corr) and (iter < params.max_val_iter):
            iter+=1
            ...
            Corr_dqn_L2_grad = (train_grads_L2 * val_grads_L2).sum() / (train_grads_L2.norm(2) * val_grads_L2.norm(2))
            Corr_dqn_L2_grad_mean = (L2_grad_vec_mean * L2_grad_val_vec_mean).sum() / (
                        L2_grad_vec_mean.norm(2) * L2_grad_val_vec_mean.norm(2))
            ...
            grad_atten_pytoch = torch.autograd.grad(((train_out1_Corr - val_out1_Corr)**2).sum(),
            ...
            print("val iter {} gama {} update attention out1 {} correlation gradL2 correlation {}".format(iter, gama, Corr_dqn_out1_mean, Corr_dqn_L2_grad_mean))
