...
def compute_td_loss(agent, num_mini_batch, mini_batch_size, replay_buffer, optimizer, gamma, loss_var_coeff, device = "CPU", train=True, compute_analytic=False, same_ind=False, start_ind_array=None):
    ...
    out_1_grad = torch.zeros((out1_states_all.size()[-1],agent.q_network.input_attention_sig.size()[0])).type(dtype)
    if compute_analytic:
...
def main_dqn(params):
    ...
    if USE_CUDA:
        device = "cuda"
    if not params.debug:
        logdir_ = 'offline_soft_OL_out1_analytic_sameB' +  params.env + '_' + str(params.seed) + '_num_arms_' + str(params.num_processes) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
        if params.rotate:
