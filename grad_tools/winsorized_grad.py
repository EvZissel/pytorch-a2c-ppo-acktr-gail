...
class WinsorizedGradDqn():
    ...
    def win_backward(self, objectives, k=1):
        '''
        calculate the gradient of the parameters

        input:
        - objectives: a list of objectives
        - k: the level of winsorization
        '''

        grads, shapes, has_grads = self._pack_grad(objectives)
        stack_grad = torch.stack(grads)
        top_grad = torch.topk(stack_grad, k=stack_grad.size(0)-k, dim=0)
        bottom_gard = torch.topk(top_grad.values, k=top_grad.values.size(0)-k, largest=False, dim=0)
        mean_flat_grad = bottom_gard.values.mean(0)
...
if __name__ == '__main__':
    ...
    net = TestNet()
    y_pred = net(x)
    grad_plot_adam = WinsorizedGradDqn(optim.Adam(net.parameters()))
