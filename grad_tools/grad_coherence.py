    ...
    def plot_backward(self, objectives):
        ...
        for g in grads:
            ...
            for i in range(len(g)):
                g_sum[i] += g[i]
                g_sum_squared[i] += (g[i]**2).sum()

        for g in grads:
            g = self._unflatten_grad(g, shapes[0])

            for i in range(len(g)):
                coherence[i] += ((g[i]*g_sum[i]).sum())/g_sum_squared[i]


        return mean_flat_grad, grads, shapes[0], coherence
