...
def compute_td_loss(agent, num_mini_batch, mini_batch_size, replay_buffer, optimizer, gamma, loss_var_coeff, train=True, start_ind_array_old=None):
    ...
    if train:
        ...
        for i in range(mini_batch_size):
            start_ind_array_old.append(start_ind_array[i])
    else:
        if len(start_ind_array_old) >= mini_batch_size:
            start_ind_array = list(start_ind_array_old)[-mini_batch_size:]
        else:
            start_ind_array = list(start_ind_array_old)
    ...
    for start_ind in start_ind_array:
        ...
        for i in range(num_processes):
            all_losses[i] += losses[i]/len(start_ind_array)
            all_grad_W2[i] += grad_W2[i]/len(start_ind_array)
            all_grad_b2[i] += grad_b2[i]/len(start_ind_array)
...
def main_dqn(params):
    ...
    start_ind_array_old = deque(maxlen=10*params.mini_batch_size_val)
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        loss, grad_L2, _ = compute_td_loss(
            agent, params.num_mini_batch, params.mini_batch_size, replay_buffer, optimizer, params.gamma, params.loss_var_coeff, train=True, start_ind_array_old=start_ind_array_old
        )
        losses.append(loss.data)

        ### Update Phi #####
        grad_loss, grad_grads_L2, _ = compute_td_loss(
            agent, params.num_mini_batch, params.mini_batch_size_val, grad_replay_buffer, optimizer, params.gamma, params.loss_var_coeff, train=False, start_ind_array_old=start_ind_array_old
        )
        grad_losses.append(grad_loss.data)

        # compute validation gradient
        val_loss, val_grads_L2, _ = compute_td_loss(
            agent, params.num_mini_batch, params.mini_batch_size_val, val_replay_buffer, optimizer, params.gamma, params.loss_var_coeff, train=False, start_ind_array_old=start_ind_array_old
        )
        ...
        dqn_grad_L2_sumQ.append(grad_grads_L2)
        ...
        if ((sum(Corr_dqn_L2_grad_vec) / len(Corr_dqn_L2_grad_vec)) < min_corr):
        # if ((sum(Corr_all_grad_vec) / len(Corr_all_grad_vec)) < min_corr) and (mean_grad_grad.norm(2) > 0.0001) and (mean_grad_val.norm(2) > 0.0001):
            ...
            grad_L2_sumQ_attn = deque(maxlen=params.max_grad_sum)
            ...
            iter = 0
            attn_grads = torch.ones_like(grad_L2)
            # for i in range(params.num_val_updates):
            while (Corr_L2_mean_attn_grad < 0.9) and iter < 10000 and attn_grads[0].norm(2) > 0.5:
            # while (Corr_L2_mean_attn_grad < 0.9) and iter < 10000:
                iter +=1
                # compute grad gradient
                grad_loss, grad_L2, _ = compute_td_loss(
                    agent, params.num_mini_batch, 10*params.mini_batch_size_val, replay_buffer, optimizer,
                    params.gamma, params.loss_var_coeff, train=False, start_ind_array_old = start_ind_array_old
                )
                grad_losses.append(grad_loss.data)
                ...
                # Corr_all_grad = (grad_mean_grad * val_mean_grad).sum() / (grad_mean_grad.norm(2) * val_mean_grad.norm(2))
                Corr_dqn_L2_grad = (grad_L2 * val_grads_L2).sum() / (grad_L2.norm(2) * val_grads_L2.norm(2))

                # grad_grad_sumQ_attn.append(grad_mean_grad)
                # grad_sumQ_val_attn.append(val_mean_grad)

                grad_L2_sumQ_attn.append(grad_L2)
                grad_sumQ_L2_val_attn.append(val_grads_L2)
                ...
                mean_grad_L2_attn = sum(grad_L2_sumQ_attn)/len(grad_L2_sumQ_attn)
                ...
                Corr_L2_mean_attn_grad = (mean_grad_L2_attn * mean_grad_val_L2_attn).sum() / (mean_grad_L2_attn.norm(2) * mean_grad_val_L2_attn.norm(2))


                val_loss = grad_loss - loss_corr_coeff*Corr_dqn_L2_grad
                # val_loss = -loss_corr_coeff*Corr_dqn_L2_grad
                ...
                print("update attention, attn L2 corr {}, mean attn L2 corr {}, grad loss {}, gard norm {}, gard val norm {}, attn_grads norm {}".format(Corr_dqn_L2_grad, Corr_L2_mean_attn_grad, grad_loss, grad_L2.norm(2),val_grads_L2.norm(2),attn_grads[0].norm(2)))
                ...
                print("attention grad {}".format(attn_grads[0].data))
        ...
        if ts % params.log_every == 0:
            ...
            summary_writer.add_scalar(f'Norms_grad/grad_norm_all', mean_dqn_grad_L2.norm(2), total_num_steps)
