...
def main_dqn(params):
    ...
    # scheduler = ReduceLROnPlateau(optimizer_val, 'min', threshold=0.000001, verbose=True)
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        # states_train = states_all.sum(0)
        # states_val = val_states_all.sum(0)
        # Corr_dqn_state = (states_train * states_val).sum() / (states_train.norm(2) * states_val.norm(2))
        # Corr_dqn_state = 0
        ...
        # if ((sum(Corr_all_grad_vec) / len(Corr_all_grad_vec)) < min_corr) and (mean_grad_grad.norm(2) > 0.0001) and (mean_grad_val.norm(2) > 0.0001):
            ...
            while (Corr_dqn_L2_grad < min_corr) and iter < 200:
                ...
                optimizer_val.step()
                # scheduler.step(Corr_dqn_L2_grad)
                # scheduler.step()
                ...
                print("iter {} update attention correlation {}".format(iter, Corr_dqn_L2_grad))
        ...
        if ts % params.log_every == 0:
            ...
            summary_writer.add_scalar(f'Correlation_grad_vec/Corr_dqn_state', Corr_dqn_L2_grad, total_num_steps)
