...
def main_dqn(params):
    ...
    if USE_CUDA:
        device = "cuda"
    if not params.debug:
        logdir_ = 'offline_soft_OL_out1_diffB_SecondOrder' +  params.env + '_' + str(params.seed) + '_num_arms_' + str(params.num_processes) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
        if params.rotate:
        ...
        wandb.init(project="main_dqn_offline_analytic", entity="ev_zisselman", config=params, name=logdir_, id=logdir_)
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        #### Update Theta #####
        loss, grads_L2, out1, start_ind_array, _ = compute_td_loss(
        ...
        val_loss, val_grads_L2, val_out1, _, _ = compute_td_loss(
            agent, params.num_mini_batch, params.mini_batch_size, val_replay_buffer, optimizer, params.gamma, params.loss_var_coeff, device, train=False,
        ...
        out1_vec.append(train_out1_Corr)
        out1_val_vec.append(val_out1_Corr)
        ...
        if (Corr_dqn_out1_mean < min_corr):
            ...
            loss, grads_L2, out1, start_ind_array, out1_grad = compute_td_loss(
                agent, params.num_mini_batch, params.mini_batch_size_val, replay_buffer, optimizer,
                params.gamma, params.loss_var_coeff, device, train=False, compute_analytic=True
            )
            ...
            val_loss, val_grads_L2, val_out1, _, out1_grad_val  = compute_td_loss(
                ...
                params.gamma, params.loss_var_coeff, device, train=False, compute_analytic=True, same_ind=True, start_ind_array=start_ind_array
            )

            train_grads_L2 = grads_L2.mean(0)
            val_grads_L2 = val_grads_L2.mean(0)
            # gradL2_vec_attn.append(train_grads_L2)
            # gradL2_val_vec_attn.append(val_grads_L2)
            # L2_grad_vec_mean = sum(gradL2_vec_attn) / len(gradL2_vec_attn)
            # L2_grad_val_vec_mean = sum(gradL2_val_vec_attn) / len(gradL2_val_vec_attn)
            Corr_dqn_L2_grad = (train_grads_L2 * val_grads_L2).sum() / (train_grads_L2.norm(2) * val_grads_L2.norm(2))
            # Corr_dqn_L2_grad_mean = (L2_grad_vec_mean * L2_grad_val_vec_mean).sum() / (L2_grad_vec_mean.norm(2) * L2_grad_val_vec_mean.norm(2))

            # out1_mean = out1.mean(0)
            # out1_val_mean = val_out1.mean(0)
            # out1_vec_attn.append(train_out1_Corr)
            # out1_val_vec_attn.append(val_out1_Corr)
            ...
            L2_loss = ((out1.mean(0) - val_out1.mean(0))**2).sum()
            grad_atten_pytoch = torch.autograd.grad(L2_loss,
                                                agent.q_network.input_attention,
                                                retain_graph=True,
                                                create_graph=True)

            # Hessian calculation
            out_1_hessian = torch.zeros((agent.q_network.input_attention.size()[0], agent.q_network.input_attention.size()[0])).type(dtype)
            for i in range(grad_atten_pytoch[0].size()[0]):
                gard_i = grad_atten_pytoch[0][i]
                gard_i_grads = torch.autograd.grad(gard_i,
                                                        agent.q_network.input_attention,
                                                        retain_graph=True)

                out_1_hessian[i,:] = gard_i_grads[0]
            ...
            # grad_atten_pytorch = grad_sig_atten_pytoch[0]*torch.sigmoid(agent.q_network.input_attention)*(1-torch.sigmoid(agent.q_network.input_attention))

            grad_loss_attn_pytorch = torch.autograd.grad(loss,
                                                        agent.q_network.input_attention,
                                                        retain_graph=True,
                                                        create_graph=True)

            out_1_hessian_loss = torch.zeros((agent.q_network.input_attention.size()[0], agent.q_network.input_attention.size()[0])).type(dtype)
            for i in range(grad_loss_attn_pytorch[0].size()[0]):
                gard_i_loss = grad_loss_attn_pytorch[0][i]
                gard_i_loss_grads = torch.autograd.grad(gard_i_loss,
                                                        agent.q_network.input_attention,
                                                        retain_graph=True)

                out_1_hessian_loss[i,:] = gard_i_loss_grads[0]

            # optimizer_val.zero_grad()
            # agent.q_network.input_attention.grad = grad_loss_attn_pytorch[0] + gama*grad_atten_pytoch[0]
            ...
            total_grad = grad_loss_attn_pytorch[0] + gama*grad_atten_pytoch[0]
            total_hesian = out_1_hessian_loss + gama*out_1_hessian

            delta_a = torch.matmul(torch.inverse(total_hesian),total_grad)
            agent.q_network.input_attention.data = agent.q_network.input_attention - delta_a
            ...
            print("grad attention pytorch {}".format(grad_atten_pytoch[0]))
            # print("grad sig attention pytorch {}".format(grad_atten_pytorch[0]))
            # print("grad attention analytic {}".format(grad_atten_analytic))
            print("grad loss attention pytorch {}".format(grad_loss_attn_pytorch[0]))
            print("delta attention {}".format(delta_a))
