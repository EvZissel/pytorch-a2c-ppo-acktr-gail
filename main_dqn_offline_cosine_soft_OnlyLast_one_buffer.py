...
def main_dqn(params):
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        #### Update Theta #####
        loss, grad_L2, _, start_ind_array = compute_td_loss(
        ...
        # if ((sum(Corr_all_grad_vec) / len(Corr_all_grad_vec)) < min_corr) and (mean_grad_grad.norm(2) > 0.0001) and (mean_grad_val.norm(2) > 0.0001):
            ...
            Corr_L2_mean_attn_grad = Mean_Corr_dqn_L2
            iter = 0
            ...
            while (Corr_L2_mean_attn_grad < 0.9) and iter < 10000:
                ...
                attn_grads_loss = torch.autograd.grad(loss,
                ...
                print("update attention, attn L2 corr {}, mean attn L2 corr {}, grad loss {}, val loss {}, gard norm {}, gard val norm {}, attn_grads norm {}".format(Corr_dqn_L2_grad, Corr_L2_mean_attn_grad, grad_loss, val_loss, grad_grads_L2.norm(2),val_grads_L2.norm(2),agent.q_network.input_attention._grad.norm(2)))
                ...
                print("grads_grad_loss {}, val_grad_loss {}, minus_grads_Corr_dqn_L2_grad {}".format(attn_grads_loss[0],attn_grads_val_loss[0] , -loss_corr_coeff*attn_grads_Corr_dqn_L2_grad[0]))
        ...
        if ts % params.log_every == 0:
            ...
            summary_writer.add_scalar(f'Norms_grad/grad_norm_all', mean_dqn_L2.norm(2), total_num_steps)
            ...
            summary_writer.add_scalar(f'Correlation_grad_vec/Corr_dqn_L2_grad', Mean_Corr_dqn_L2, total_num_steps)
