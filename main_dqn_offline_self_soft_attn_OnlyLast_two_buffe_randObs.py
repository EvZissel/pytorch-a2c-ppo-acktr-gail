...
from models_dqn import DQN, CnnDQN, DQN_softAttn, DQN_softAttn_L2grad, DQN_RNNLast, DQN_RNNLast_analytic, DQN_self_attention_oracel
...
def evaluate(agent, eval_envs_dic ,env_name, eval_locations_dic, num_processes, num_tasks, **kwargs):
    ...
    for iter in range(0, num_tasks, num_processes):
        ...
        for t in range(kwargs["steps"]):
            with torch.no_grad():
                actions, recurrent_hidden = agent.act(obs, obs, recurrent_hidden, epsilon=-1, masks=masks)
...
def main_dqn(params):
    ...
    q_network = DQN_self_attention_oracel(envs.observation_space.shape, envs.action_space.n, params.zero_ind, trajectory_len=params.task_steps, recurrent=True, hidden_size=params.hidden_size, attn_hidden_size=params.attn_hidden_size)
    ...
    scheduler = MultiStepLR(optimizer, milestones=[4000, 6000], gamma=0.1)
    ...
    grad_out1_val_vec = []
    for i in range(len(attention_parameters)):
        atten_grad_Loss.append(deque(maxlen=int(params.mini_batch_size_val / params.mini_batch_size)))
        grad_out1_vec.append(deque(maxlen=int(params.mini_batch_size_val/params.mini_batch_size)))
        grad_out1_val_vec.append(deque(maxlen=int(params.mini_batch_size_val/params.mini_batch_size)))
    ...
    for ts in range(params.continue_from_epoch, params.continue_from_epoch+num_updates):
        ...
        loss, grads_L2, out1, start_ind_array, out1_grad = compute_td_loss(
            agent, params.num_mini_batch, params.mini_batch_size, replay_buffer, optimizer, params.gamma, params.loss_var_coeff, device, train=True, compute_analytic=compute_analytic,
        )
        ...
        #     optimizer_val.step()
            ...
            # print("attention query {}".format(q_network.query_attention.data))
        ...
        if ts % params.log_every == 0:
            # out_str = "Iter {}, Timestep {}, key attention {}, query attention {}".format(ts, total_num_steps, torch.transpose(q_network.key_attention,1,0).data, q_network.query_attention.data)
            ...
            if len(episode_rewards) > 1:
                ...
                for eval_disp_name, eval_env_name in EVAL_ENVS.items():
                    ...
                    # print(f'attention sum {eval_disp_name}: {(input_attention.sum(0)).data.cpu().numpy()}')
                    if not params.debug:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attn_log_every", type=int, default=1)
    ...
    parser.add_argument("--attn_hidden_size", type=int, default=32)
