...
class VideoRecorderprocess(VideoRecorderWrapper):
    def __init__(self, env, directory, ob_key, prefix):
        super().__init__(env=env, directory=directory, ob_key=ob_key, prefix=prefix)
...
test_env = VideoRecorderprocess(env=test_env, directory="./videos", ob_key="rgb", prefix = str(test_start_level) + '_ChangedRewardWrnn3')
...
# Load previous model
continue_from_epoch = 6098
# save_dir = '/home/ev/Desktop/pytorch-a2c-ppo-acktr-gail/ppo_log/maze_seed_0_noRNN/maze_seed_0_num_env_128_entro_0.01_02-10-2021_18-10-29_noRNN'
save_dir = '/home/ev/Desktop/pytorch-a2c-ppo-acktr-gail/ppo_log/maze_seed_0_num_env_128_entro_0.01_11-09-2021_00-40-14'
...
while not done[0]:
    with torch.no_grad():
        ...
        _, action, _, eval_recurrent_hidden_states, _, _, _, _ = actor_critic.act(
            torch.FloatTensor(obs).float().to(device),
    ...
    # obs = obs['rgb'].transpose(0, 3, 1, 2) / 255.0
    # Change reward location
    obs = obs['rgb'][0]
    indexes = np.stack([(obs[:, :, 2] == 255), (obs[:, :, 2] == 255), (obs[:, :, 2] == 255)], axis=2)
    obs = obs * (1 - indexes)
