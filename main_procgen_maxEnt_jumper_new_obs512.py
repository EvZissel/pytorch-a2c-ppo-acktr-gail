...
from evaluation import evaluate_procgen, evaluate_procgen_maxEnt, evaluate_procgen_maxEnt_org, evaluate_procgen_maxEnt_L2, evaluate_procgen_maxEnt_avepool
...
def main():
    ...
    logdir_ = args.env_name + '_seed_' + str(args.seed) + '_num_env_' + str(args.num_level) + '_entro_512' + str(args.entropy_coef) + '_gama_' + str(args.gamma) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
    ...
    down_sample_avg = nn.AvgPool2d(25, stride=25)
    for eval_disp_name in EVAL_ENVS:
        for i in range(args.num_test_level):
            ...
            # obs = envs.reset()
            obs = torch.tensor(envs.env.get_info()[0]['rgb']/255).transpose(0, 2).unsqueeze(0)
            # obs_sum = obs
            # plot mazes
    ...
    full_obs_observation_space = (envs_full_obs.env._info['rgb'].shape[3], envs_full_obs.env._info['rgb'].shape[1], envs_full_obs.env._info['rgb'].shape[2])
    # rollout storage for agent
    rollouts = RolloutStorage(args.num_steps, args.num_processes,
                              envs.observation_space.shape, full_obs_observation_space, envs.action_space,
    ...
    logger = maxEnt_Logger(args.num_processes, max_reward_seeds, start_train_test, envs.observation_space.shape, full_obs_observation_space,
    ...
    full_obs = envs_full_obs.env.get_info()
    rollouts.obs[0].copy_(obs)
    for i in range(args.num_processes):
        obs_full_i = torch.tensor(full_obs[i]['rgb'] / 255).transpose(0, 2)
        rollouts.obs_full[i].copy_(obs_full_i)
        rollouts.obs_sum[i].copy_(obs_full_i)
        rollouts.obs0[i].copy_(obs_full_i)
    # rollouts.to(device)
    ...
    obs_train_full = eval_envs_dic_full_obs['train_eval'].env.get_info()
    for i in range(args.num_processes):
        obs_train_full_i = torch.tensor(obs_train_full[i]['rgb'] / 255).transpose(0, 2)
        logger.obs_full['train_eval'][i].copy_(obs_train_full_i)
        logger.obs_sum['train_eval'][i].copy_(obs_train_full_i)
        logger.obs0['train_eval'][i].copy_(obs_train_full_i)
    ...
    obs_test_full = eval_envs_dic_full_obs['test_eval'].env.get_info()
    for i in range(args.num_processes):
        obs_test_full_i = torch.tensor(obs_test_full[i]['rgb'] / 255).transpose(0, 2)
        logger.obs_full['test_eval'][i].copy_(obs_test_full_i)
        logger.obs_sum['test_eval'][i].copy_(obs_test_full_i)
        logger.obs0['test_eval'][i].copy_(obs_test_full_i)
    ...
    for j in range(args.continue_from_epoch, args.continue_from_epoch+num_updates):
        ...
        for step in range(args.num_steps):
            ...
            _, _, _, _ = envs_full_obs.step(action.squeeze().cpu().numpy())
            obs_full_list = envs_full_obs.env.get_info()
            obs_full = torch.zeros_like(rollouts.obs_sum)
            for i in range(len(done)):
                obs_full[i] = torch.tensor(obs_full_list[i]['rgb'] / 255).transpose(0, 2)


            # if max(reward) < 10 and max(reward) >0:
            #     print(reward)
            int_reward = np.zeros_like(reward)
            next_obs_sum = rollouts.obs_sum + obs_full.cpu()
            next_obs_sum = down_sample_avg(next_obs_sum)
            obs_sum = down_sample_avg(rollouts.obs_sum)
            for i in range(len(done)):
            ...
            rollouts.insert(obs, recurrent_hidden_states, action,
                            action_log_prob, value, torch.from_numpy(int_reward).unsqueeze(1), masks, bad_masks, attn_masks, attn_masks1, attn_masks2, attn_masks3, seeds, infos, obs_full)
        ...
        if ((args.eval_interval is not None and j % args.eval_interval == 0) or j == args.continue_from_epoch):
            ...
            for eval_disp_name in EVAL_ENVS:
                eval_dic_rew[eval_disp_name], eval_dic_int_rew[eval_disp_name], eval_dic_done[eval_disp_name], eval_dic_seeds[eval_disp_name] = evaluate_procgen_maxEnt_avepool(actor_critic, eval_envs_dic, eval_envs_dic_full_obs, eval_disp_name,
                                                  args.num_processes, device, args.num_steps, logger, kernel_size=25, stride=25)
