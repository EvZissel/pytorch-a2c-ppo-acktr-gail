...
from evaluation import evaluate_procgen, evaluate_procgen_maxEnt, evaluate_procgen_ensemble
...
def main():
    ...
    # logdir_ = 'Ensemble_' + args.env_name + '_seed_' + str(args.seed) + '_num_env_' + str(args.num_level) + '_entro_' + str(args.entropy_coef) + '_gama_' + str(args.gamma) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
    ...
    #                           eval_envs_dic['train_eval'].observation_space.shape, eval_envs_dic['train_eval'].action_space,
    ...
    #     actor_critic_0.load_state_dict(actor_critic_weighs['state_dict'])
    #     agent_1.optimizer.load_state_dict(actor_critic_weighs['optimizer_state_dict'])



    # Load previous model
    if (args.saved_epoch0 > 0) and args.save_dir0 != "":
        save_path = args.save_dir0
        actor_critic_weighs = torch.load(
            os.path.join(save_path, args.load_env_name + "-epoch-{}.pt".format(args.saved_epoch0)), map_location=device)
        actor_critic_0.load_state_dict(actor_critic_weighs['state_dict'])
    ...
    if (args.saved_epoch1 > 0) and args.save_dir1 != "":
        ...
        actor_critic_1.load_state_dict(actor_critic_weighs['state_dict'])
    ...
    if (args.saved_epoch2 > 0) and args.save_dir2 != "":
        ...
        actor_critic_2.load_state_dict(actor_critic_weighs['state_dict'])

    logger = maxEnt_Logger(args.num_processes, max_reward_seeds, start_train_test, eval_envs_dic['train_eval'].observation_space.shape,
    ...
    for eval_disp_name in EVAL_ENVS:
        eval_dic_rew[eval_disp_name], eval_dic_int_rew[eval_disp_name], eval_dic_done[eval_disp_name], eval_dic_seeds[eval_disp_name]  = evaluate_procgen_ensemble(actor_critic_0, actor_critic_1, actor_critic_2,
